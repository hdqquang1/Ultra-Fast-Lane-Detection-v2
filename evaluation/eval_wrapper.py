                ...
                if valid[j, :, i].sum() > 2:
                    for k in range(valid.shape[1]):
                        if valid[j, k, i]:
                            fp.write(
                                "%.3f %.3f "
                                % (loc[j, k, i], culane_row_anchor[k] * 590)
                            )
                    fp.write("\n")
                ...
                if valid[j, :, i].sum() > 2:
                    for k in range(valid.shape[1]):
                        if valid[j, k, i]:
                            fp.write(
                                "%.3f %.3f "
                                % (culane_col_anchor[k] * 1640, loc[j, k, i])
                            )
                    fp.write("\n")


def generate_lines_local(
    dataset, out, out_ext, names, output_path, mode="normal", row_anchor=None
):
                    ...
                    for k in range(valid.shape[1]):
                        if valid[j, k, i]:
                            all_ind = torch.tensor(
                                list(
                                    range(
                                        max(0, max_indices[j, k, i] - local_width),
                                        min(
                                            out.shape[1] - 1,
                                            max_indices[j, k, i] + local_width,
                                        )
                                        + 1,
                                    )
                                )
                            ...
                            out_tmp = (
                                out[j, all_ind, k, i].softmax(0) * all_ind.float()
                            ).sum() + 0.5
                ...
                if valid[j, :, i].sum() > num_cls / 4:
                    for k in range(valid.shape[1]):
                        if valid[j, k, i]:
                            ...
                            out_tmp = (
                                out_col[j, all_ind, k, i].softmax(0) * all_ind.float()
                            ).sum() + 0.5
                            if dataset == "CULane":
...
def generate_lines_local_curve_combine(
    dataset, out, out_ext, names, output_path, mode="normal", row_anchor=None
):
    batch_size, num_grid_row, num_cls, num_lane = out.shape
    max_indices = out.argmax(1).cpu()
...
def generate_lines_reg(
    out, out_ext, names, output_path, mode="normal", row_anchor=None
):
    batch_size, num_grid_row, num_cls, num_lane = out.shape
    # n , num_cls, num_lanes
...
def generate_lines_col_reg(
    out_col, out_col_ext, names, output_path, mode="normal", col_anchor=None
):
    batch_size, num_grid_col, num_cls, num_lane = out_col.shape
    # max_indices = out_col.argmax(1).cpu()
    # n, num_cls, num_lanes
                ...
                if valid[batch_idx, :, lane_idx].sum() >= min_lane_length:
                    ...
                    for cls_idx in range(num_cls):
                        ...
                        if valid[batch_idx, cls_idx, lane_idx]:
                            all_ind = torch.tensor(
                                list(
                                    range(
                                        max(
                                            0,
                                            max_indices[batch_idx, cls_idx, lane_idx]
                                            - local_width,
                                        ),
                                        min(
                                            num_grid - 1,
                            ...
                            out_tmp = (
                                loc_row[batch_idx, all_ind, cls_idx, lane_idx].softmax(
                                    0
                                )
                                * all_ind.float()
                            ).sum() + 0.5
                        ...
                        if valid_left[batch_idx, cls_idx, lane_idx]:
                            ...
                            out_tmp_left = (
                                loc_row_left[
                                    batch_idx, all_ind_left, cls_idx, lane_idx
                                ].softmax(0)
                                * all_ind_left.float()
                            ).sum() + 0.5
                            out_tmp_left = (
                                out_tmp_left / (num_grid - 1) * 1640 + 1640.0 / 25
                            )
                            cnt += 1
                        ...
                        if valid_right[batch_idx, cls_idx, lane_idx]:
                            ...
                            out_tmp_right = (
                                loc_row_right[
                                    batch_idx, all_ind_right, cls_idx, lane_idx
                                ].softmax(0)
                                * all_ind_right.float()
                            ).sum() + 0.5
                            out_tmp_right = (
                                out_tmp_right / (num_grid - 1) * 1640 - 1640.0 / 25
                            )
                            cnt += 1
                        ...
                        if cnt >= 2:
                            pt_all.append(
                                (out_tmp_all / cnt, row_anchor[cls_idx] * 590)
                            )
                ...
                if valid[batch_idx, :, lane_idx].sum() >= min_lane_length:
                    ...
                    for cls_idx in range(num_cls):
                        ...
                        if valid[batch_idx, cls_idx, lane_idx]:
                            ...
                            out_tmp = (
                                loc_col[batch_idx, all_ind, cls_idx, lane_idx].softmax(
                                    0
                                )
                                * all_ind.float()
                            ).sum() + 0.5
                        ...
                        if valid_up[batch_idx, cls_idx, lane_idx]:
                            all_ind_up = torch.tensor(
                                list(
                                    range(
                                        max(
                                            0,
                                            max_indices_up[batch_idx, cls_idx, lane_idx]
                                            - local_width,
                                        ),
                                        min(
                                            num_grid - 1,
                            ...
                            out_tmp_up = (
                                loc_col_up[
                                    batch_idx, all_ind_up, cls_idx, lane_idx
                                ].softmax(0)
                                * all_ind_up.float()
                            ).sum() + 0.5
                            out_tmp_up = (
                                out_tmp_up / (num_grid - 1) * 590 + 32.0 / 534 * 590
                            )
                            cnt += 1
                        ...
                        if valid_down[batch_idx, cls_idx, lane_idx]:
                            ...
                            out_tmp_down = (
                                loc_col_down[
                                    batch_idx, all_ind_down, cls_idx, lane_idx
                                ].softmax(0)
                                * all_ind_down.float()
                            ).sum() + 0.5
                            out_tmp_down = (
                                out_tmp_down / (num_grid - 1) * 590 - 32.0 / 534 * 590
                            )
                            cnt += 1
                        ...
                        if cnt >= 2:
                            pt_all.append(
                                (col_anchor[cls_idx] * 1640, out_tmp_all / cnt)
                            )
                    if len(pt_all) < min_lane_length:
                        continue
                    for pt in pt_all:
...
def run_test_tta(
    ...
    for i, data in enumerate(dist_tqdm(loader)):
        ...
        with torch.no_grad():
            ...
            exist_row, exist_row_left, exist_row_right, _, _ = torch.chunk(
                pred["exist_row"], 5
            )
            exist_col, _, _, exist_col_up, exist_col_down = torch.chunk(
                pred["exist_col"], 5
            )
        ...
        if row_valid[:, row_lane_idx].sum() > min_lanepts_row:
            ...
            for row_cls_idx in range(row_num_cls):
                if row_valid[row_cls_idx, row_lane_idx]:
                    all_ind = torch.tensor(
                        list(
                            range(
                                max(
                                    0,
                                    row_max_indices[row_cls_idx, row_lane_idx]
                                    - local_width_row,
                                ),
                                min(
                                    row_num_grid - 1,
                    ...
                    coord = (
                        row_out[all_ind, row_cls_idx, row_lane_idx].softmax(0)
                        * all_ind.float()
                    ).sum() + 0.5
        ...
        if col_valid[:, col_lane_idx].sum() > min_lanepts_col:
            ...
            for col_cls_idx in range(col_num_cls):
                if col_valid[col_cls_idx, col_lane_idx]:
                    all_ind = torch.tensor(
                        list(
                            range(
                                max(
                                    0,
                                    col_max_indices[col_cls_idx, col_lane_idx]
                                    - local_width_col,
                                ),
                                min(
                                    col_num_grid - 1,
                    ...
                    coord = (
                        col_out[all_ind, col_cls_idx, col_lane_idx].softmax(0)
                        * all_ind.float()
                    ).sum() + 0.5
...
def eval_lane(net, cfg, ep=None, logger=None):
    ...
    if cfg.dataset == "CurveLanes":
        ...
        if is_main_process():
            res = call_curvelane_eval(
                cfg.data_root, "curvelanes_eval_tmp", cfg.test_work_dir
            )
            TP, FP, FN = 0, 0, 0
...
def call_culane_eval(data_dir, exp_name, output_path):
    ...
    list9 = os.path.join(data_dir, "list/test.txt")
    ...
    out9 = os.path.join(output_path, "txt", "out.txt")
